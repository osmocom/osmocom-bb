...
int cursor = 0, scroll_apps = 0, display_addr = 0;
...
int _memcmp(char *s1, char *s2, int size)
{
	int i;
...
static void refresh_display(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (!display_addr)
			...
			memcpy(text, apps[scroll_apps + i].name, 2);
			sprintf(text + 2, "0x%06x",
				(unsigned int) apps[scroll_apps + i].start);
			fb_putstr(text, framebuffer->width);
		}
		if (scroll_apps + i == cursor) {
			fb_setfg(FB_COLOR_BLACK);
...
//	display_unset_attr(DISP_ATTR_INVERT);
	...
	while (1) {
		for (i = 0; i < 50; i++) {
			...
			keypad_poll();
			jiffies++; /* hack: make timers run at some speed */
			osmo_timers_update();
			handle_key_code();
		}
		refresh_display();
